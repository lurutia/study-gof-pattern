...
import com.gosuljo.gof.infra.factory.abstract_factory.after.WhiteCarPartsFactory;
...
public class AbstractFactoryMethodTests {
    ...
    // 추상 팩토리 메소드 테스트
    @Test
    public void abstract_factory_method_2() {
        Car whiteCar = new com.gosuljo.gof.infra.factory.abstract_factory.after.WhiteCarFactory(new WhiteCarPartsFactory()).orderCar("whiteCar", "white@email.com");
        assertEquals(whiteCar.getName(), "whiteCar");
        assertEquals(whiteCar.getLogo(), "\uD83D\uDEE5");
        assertEquals(whiteCar.getColor(), "white");
        assertThat(whiteCar.getHandle(), instanceOf(WhiteHandle.class));
